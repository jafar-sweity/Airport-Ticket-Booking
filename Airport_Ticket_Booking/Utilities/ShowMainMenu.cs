...
namespace Airport_Ticket_Booking.Utilities
{
    class Menu
    {
        ...
        public Menu(IBookingService bookingService, IFlightRepository flightRepository, IPassengerService passengerService)
        {
        ...
        private void BookFlight()
        {
            try
            {
                ...
                if (flightRepository.Count > 0)
                {
                    ...
                    {
                        Console.WriteLine($"Flight Number: {flying.FlightNumber}, {flying.DepartureCountry}->{flying.DestinationCountry} , Departure: {flying.DepartureAirport} -> Arrival: {flying.ArrivalAirport}, Date: {flying.DepartureTime.ToShortDateString()}, Price: {flying.BasePrice}");
                    }
                }
                else
                {
        ...
        private void ViewPersonalBookings()
        {
            try
            {
                ...
                if (bookings.Count > 0)
                {
                    ...
                    {
                        Console.WriteLine($"Booking ID: {booking.BookingID}, Flight ID: {booking.Flight}, Class: {booking.FlightClass}, Price: {booking.Price}");
                    }
                }
                else
                {
        ...
        private void CancelBooking()
        {
            try
            {
                ...
                foreach (var booking in bookings)
                {
                    Console.WriteLine($"Booking ID: {booking.BookingID}, Flight Number: {booking.Flight}, Class: {booking.FlightClass}, Price: {booking.Price}");
                }
                ...
                while (true)
                {
                    ...
                    if (int.TryParse(Console.ReadLine(), out flightClassOption))
                    {
                        if (flightClassOption >= 0 && flightClassOption <= 2)
                        {
                            ...
                            bookingToModify.FlightClass = newFlightClass;
                            break;
                        }
                        else
                        {
        ...
        private void SearchAvailableFlights()
        {
            try
            {
                ...
                if (flights.Count > 0)
                {
                    ...
                    {
                        Console.WriteLine($"Flight Number: {flight.FlightNumber}, Departure: {flight.DepartureAirport} -> Arrival: {flight.ArrivalAirport}, Date: {flight.DepartureTime.ToShortDateString()}, Price: {flight.BasePrice}");
                    }
                }
                else
                {
        ...
        private void ViewAllAvailableFlights()
        {
            try
            {
                ...
                if (flights.Count > 0)
                {
                    ...
                    foreach (var flight in flights)
                    {
                        Console.WriteLine($"Flight Number: {flight.FlightNumber}, {flight.DepartureCountry}->{flight.DestinationCountry} ,Departure: {flight.DepartureAirport} -> Arrival: {flight.ArrivalAirport}, Date: {flight.DepartureTime.ToShortDateString()}, Price: {flight.BasePrice}");
                    }
                }
                else
                {
